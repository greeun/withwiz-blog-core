 ...
 * Prisma의 $queryRaw를 활용하여 매개변수화된 안전한 쿼리를 수행한다.
 *
 * 주의: 이 서비스는 PostgreSQL 전용이며, unaccent 확장과
 * 선택적으로 blog_posts.search_vector 생성 컬럼을 활용할 수 있다.
 * 마이그레이션은 packages/blog-core/prisma/migrations/fulltext-search.sql 참고.
 *
 */
...
export function createSearchService(
  ...
  return {
    ...
    async search(options: SearchOptions): Promise<PaginatedResult<SearchResult>> {
      ...
      // 빈 검색어는 빈 결과를 반환 (호출 측에서 분기하도록 유도)
      if (!tsQuery) {
      ...
      // 생성 컬럼은 to_tsvector('simple', unaccent(...))로 정의되어 있으므로,
      // 쿼리 측에도 unaccent를 적용해야 인덱스가 활용된다.
      // (migration: packages/blog-core/prisma/migrations/fulltext-search.sql)
      const listSql = `
        SELECT

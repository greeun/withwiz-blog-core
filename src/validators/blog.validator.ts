 ...
 *
 * createBlogSchemas(config?) 팩토리로 maxAttachments 등을 설정에서 주입 가능
 * 정적 스키마(CreateBlogPostSchema 등)는 기본값(max 5)으로 제공
 */
import { z } from 'zod';
 ...
 *
 * 정규식은 `utils/slug.ts`의 SLUG_PATTERN을 재사용한다(단일 진실 공급원).
 */
...
export function createBlogSchemas(config?: BlogSchemaConfig): BlogSchemas {
  ...
  const CreateSchema = z.object({
    title: z.string().min(1, t.adminTitleRequired).max(200),
    content: z.string().min(1, t.adminContentRequired),
    excerpt: z.string().max(500).optional(),
    category: z.string().min(1, t.adminCategoryLabel),
    coverImageUrl: optionalUrlSchema,
    coverImageKey: z.string().optional(),
    attachments: z.array(attachmentSchema).max(maxAttachments).optional().default([]),
  ...
  const UpdateSchema = z.object({
    ...
    attachments: z.array(attachmentSchema).max(maxAttachments),
    featured: z.boolean(),
    published: z.boolean(),
  ...
  const BulkSchema = z.object({
    ids: z.array(z.string()).min(1, t.errorValidation),
...
// ── 정적 기본 스키마 (기본값 maxAttachments=5) ──

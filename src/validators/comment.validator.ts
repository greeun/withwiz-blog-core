...
export function createCommentSchemas(config?: CommentSchemaConfig): CommentSchemas {
  ...
  const Create = z.object({
    postId: z.string().min(1, t.errorValidation),
    parentId: z.string().optional(),
    content: z
      .string()
      .min(1, t.commentFormContentLabel)
      .max(maxContent, t.commentMaxLengthHint),
    guestName: z
      .string()
      .min(1, t.commentFormNameLabel)
      .max(maxGuestName, t.errorValidation)
      .optional(),
    guestEmail: z.string().email(t.commentFormEmailLabel).optional(),
